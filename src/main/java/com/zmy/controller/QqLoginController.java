...
public class QqLoginController {
    ...
    public String qqLogin(HttpServletRequest request, String backurl) {
        try {

            url = backurl;
            log.info("请求url" + url);
            log.info("请求地址:" + new Oauth().getAuthorizeURL(request));
    ...
    public String Login(HttpServletRequest request, HttpSession session/*, Model model,@PageableDefault(size = 8, sort = {"updateTime"}, direction = Sort.Direction.DESC) Pageable pageable*/) {
...
//                做一些数据统计工作
                ...
                log.info("<a href=" + "/shuoshuoDemo.html" + " target=\"_blank\">去看看发表说说的demo吧</a>");
                ...
                if (userInfoBean.getRet() == 0) {
                    ...
                    log.info("<image src=" + userInfoBean.getAvatar().getAvatarURL30() + "/><br/>");
                    log.info("<image src=" + userInfoBean.getAvatar().getAvatarURL50() + "/><br/>");
                    log.info("<image src=" + userInfoBean.getAvatar().getAvatarURL100() + "/><br/>");
                } else {
                    log.error("很抱歉，我们没能正确获取到您的信息，原因是： " + userInfoBean.getMsg());
                }
                log.info("<p> end -----------------------------------利用获取到的accessToken,openid 去获取用户在Qzone的昵称等信息 ---------------------------- end </p>");


                log.info("<p> start ----------------------------------- 验证当前用户是否为认证空间的粉丝------------------------------------------------ start <p>");
                ...
                if (pageFansBean.getRet() == 0) {
                    log.info("<p>验证您" + (pageFansBean.isFans() ? "是" : "不是") + "QQ空间97700000官方认证空间的粉丝</p>");
                } else {
                    log.error("很抱歉，我们没能正确获取到您的信息，原因是： " + pageFansBean.getMsg());
                }
                log.info("<p> end ----------------------------------- 验证当前用户是否为认证空间的粉丝------------------------------------------------ end <p>");


                log.info("<p> start -----------------------------------利用获取到的accessToken,openid 去获取用户在微博的昵称等信息 ---------------------------- start </p>");
                ...
                if (weiboUserInfoBean.getRet() == 0) {
                    //获取用户的微博头像----------------------start
                    log.info("<image src=" + weiboUserInfoBean.getAvatar().getAvatarURL30() + "/><br/>");
                    log.info("<image src=" + weiboUserInfoBean.getAvatar().getAvatarURL50() + "/><br/>");
                    log.info("<image src=" + weiboUserInfoBean.getAvatar().getAvatarURL100() + "/><br/>");
                    //获取用户的微博头像 ---------------------end

                    //获取用户的生日信息 --------------------start
                    log.info("<p>尊敬的用户，你的生日是： " + weiboUserInfoBean.getBirthday().getYear()
                    ...
                    log.error("很抱歉，我们没能正确获取到您的信息，原因是： " + weiboUserInfoBean.getMsg());
                }

                log.info("<p> end -----------------------------------利用获取到的accessToken,openid 去获取用户在微博的昵称等信息 ---------------------------- end </p>");
    ...
    @RequestMapping("/qqlogout")
    public String qqLogout(HttpSession session) {
        session.removeAttribute("qquser");
        return "redirect:" + url;
    }
